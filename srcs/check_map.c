...
//最初の二行だけ謎のチェックそしている。-3はなんだろう。-1分は改行の分、残りは。。？
...
//mapが適切かチェック。引数のmapはmapファイルを二次元配列にしたもの。heightは事前に測定していたmapの高さ
char	**check_map(char **map, int height)
{
	if (!(check_first_2_lines(map)))//一行目が適切か、mapの左上が適切か
		return (NULL);
	if (!(check_nb_lines(map, height)))//高さが適切か、、、？
		return (NULL);
	if (!(check_line_length_consistency(map)))//幅が全ての行で一緒か
		return (NULL);
	if (!(check_charset_consistency(map)))//全ての文字がempty文字かobstacle文字であるかチェック
		return (NULL);
	return (map);
}
